...
function book(title, author, pages, status) {
  ...
  this.status = status;
  this.info = () =>
    `${this.title} by ${this.author} with ${this.pages} pages. ${this.status}`;
}
...
function updateStats(status, status1) {
  ...
  document.querySelector(
    '#total-books'
  ).textContent = `Books: ${totalBooksCounter}`;
  document.querySelector(
    '#total-read'
  ).textContent = `Books Read: ${totalReadCounter}`;
  document.querySelector(
    '#total-reading'
  ).textContent = `Books Reading: ${totalReadingCounter}`;
  document.querySelector(
    '#total-not-read'
  ).textContent = `Books to Read: ${totalNotReadCounter}`;
}
